...
// dynamopolicy represents the dbItem command
var dynamopolicy = &cobra.Command{
	Use:   "dbItem",
	Short: "Create an S3 Bucket Policy!",
...
func init() {
	rootCmd.AddCommand(dynamopolicy)

	// Here you will define your flags and configuration settings.

	// Cobra supports Persistent Flags which will work for this command
	// and all subcommands, e.g.:
	// dynamopolicy.PersistentFlags().String("foo", "", "A help for foo")

	// Cobra supports local flags which will only run when this command
	// is called directly, e.g.:
	// dynamopolicy.Flags().BoolP("toggle", "t", false, "Help message for toggle")
}
